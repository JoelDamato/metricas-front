...
const Dashboard = () => {
  ...
  return (
    <div className="p-4 bg-gray-100 min-h-screen overflow-x-auto">
      ...
      {monthlyRankings.map(({ month, ranking, facturacion, totals }) => (
        ranking.length > 0 && (
          <div key={month} className="bg-white shadow-lg rounded-lg p-4 mb-6 overflow-x-auto">
            ...
            <p className="text-md text-gray-700 font-semibold text-center">Facturación Total: {facturacion.toLocaleString("es-ES", { style: "currency", currency: "USD" })}</p>
            <table className="w-full table-auto text-left text-gray-700 text-sm md:text-base">
              <thead>
                <tr className="bg-gray-200">
                  <th className="md:px-4"></th>
                  <th className="py-1 px-1 md:px-4">Closer</th>
                  <th className="py-2 px-2 md:px-4">Cash Collected</th>
                  <th className="py-2 px-2 md:px-4">Venta</th>
                  <th className="py-2 px-2 md:px-4">Facturación</th>
                </tr>
              </thead>
              <tbody>
                {ranking.map((item, index) => (
                  <tr key={index} className="hover:bg-gray-100 cursor-pointer" onClick={() => setSelectedCloser(item)}>
                    <td className=" sm:px-4 border-b">{index + 1}</td>
                    <td className="py-1 px-1 md:px-4 border-b">{item.closer}</td>
                    <td className="py-1 px-1 md:px-4 border-b">{item.cashCollected.toLocaleString("es-ES", { style: "currency", currency: "USD" })}</td>
                    <td className="py-1 px-1 md:px-4 border-b">{item.offersWon}</td>
                    <td className="py-1 px-1 md:px-4 border-b">{item.facturacion.toLocaleString("es-ES", { style: "currency", currency: "USD" })}</td>
                  </tr>
                ))}
                {/* Fila de totales */}
                <tr className="bg-gray-100 font-bold">
                  <td className="py-2 px-2 md:px-4 border-t">Total</td>
                  <td className="py-2 px-2 md:px-4 border-t"></td>
                  <td className="py-2 px-2 md:px-4 border-t">
                    {totals?.cashCollected?.toLocaleString("es-ES", { style: "currency", currency: "USD" }) || "N/A"}
                  </td>
                  <td className="py-2 px-2 md:px-4 border-t">
                    {totals?.offersWon || "N/A"}
                  </td>
                  <td className="py-2 px-2 md:px-4 border-t">
                    {totals?.facturacion?.toLocaleString("es-ES", { style: "currency", currency: "USD" }) || "N/A"}
                  </td>
                </tr>
              </tbody>
            </table>
